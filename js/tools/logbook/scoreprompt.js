...
// A goal that lives at a bookmark (WSHED-108) opens on that page.
...
export async function offerScore(goalId) {
  const place = logbook.placeForGoal(goalId);
  if (!place) return false;
  const { score: s, page, mark } = place;
  ...
  const { body, close, closed } = openSheet({
    ...
    html: `
      <ul class="lb-acct-list">
        <li><button type="button" class="lb-acct-row" id="lb-score-open">${icon("score")}<span><b>open ${esc(s.title)}</b><small>${page ? `at ${esc(mark.label)} · page ${page}` : `${s.pages} page${s.pages === 1 ? "" : "s"}`} · ${here ? "the clock keeps running" : "downloads from your cloud space first"}</small></span></button></li>
  ...
  body.querySelector("#lb-score-open").addEventListener("click", () => { opened = true; haptic(); close(); location.hash = `#/scores/${encodeURIComponent(s.id)}${page ? `?p=${page}` : ""}`; });
