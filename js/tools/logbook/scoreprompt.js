...
// or when its file is on another device (nothing to open yet).
...
export async function offerScore(goalId) {
  ...
  if (!scoreStore.has(s.id)) return false;
  let opened = false;
  const { body, close, closed } = openSheet({
    ...
    html: `
      <ul class="lb-acct-list">
        <li><button type="button" class="lb-acct-row" id="lb-score-open">${icon("score")}<span><b>open ${esc(s.title)}</b><small>${s.pages} page${s.pages === 1 ? "" : "s"} · the clock keeps running</small></span></button></li>
