 ...
 * steps from its own alter; one already at the end stays while the others move. The step sets
 * the alter and clears any explicit sign (v117, WSHED-155 — Leif: no redundant naturals): the
 * layout draws a sign only where the bar needs one, so a step back to the key in G writes a bare
 * D, and a natural only after a D♯ earlier in the bar. A courtesy sign is the rail's ♮ button's
 * job. Nudge when no selected note is a note, or none can move.
 */
export function stepAccidental(doc, items, dir) {
  ...
  for (const { ps } of ts) {
    for (const p of ps) {
      ...
      p.alter = alter; delete p.acc; moved++;
    }
  }
