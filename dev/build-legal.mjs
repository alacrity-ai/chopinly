...
<body>
  <header class="l-top">
    <a class="l-brand" href="/" aria-label="Chopinly"><img src="/brand/chopinly-mark.svg" alt="" width="32" height="32"><span>Chopinly</span></a>
...
const privacy = `
    ...
    <p>If you sign in, we store the items in §3 so that your practice is backed up and available on your other devices. Signing in is optional and reversible: sign out keeps your data on the device, <em>delete account</em> removes it from our servers. Score files are the one thing that is not uploaded by signing in alone: they go up only when you choose <em>upload</em> in Scores.</p>
    ...
    <div class="l-table-wrap"><table>
      ...
      <tbody>
        ...
        <tr><td><b>Practice data</b></td><td>Goals (name, optional composer, type, status), practice segments (start and end times, optional tempo), notes you write, the <em>list</em> of takes you record (which goal, when, how long, a 48-number loudness outline, whether you starred it), the <em>list</em> of your scores (title, composer, tags, page count, size, a hash of the file, when you last opened it), your bookmarks and the ink you draw on pages (as vector strokes), and deletion markers so devices agree.</td><td>Backup and sync — the service you asked for.</td><td>Until you delete the item or your account.</td></tr>
        ...
        <tr><td><b>Score files you upload</b></td><td>The PDF of a score, exactly as you imported it, when — and only when — you choose <em>upload</em> in Scores. Stored in a private object store (Cloudflare R2) under your account, never shared, listed or published. Each account has a storage allowance shown in the account sheet (100 MB while the feature is promotional; the limit is a setting on your account, not a promise).</td><td>To open the same score on your other devices.</td><td>Until you remove it from the cloud, delete the score, or delete your account.</td></tr>
    ...
    <p>There are no passwords: Chopinly signs you in with a one-time code sent to your email. We hold no payment details because there is nothing to pay for; an account's plan (which sets its storage allowance) is a single word on the account record. We do not ask for your name, age, location, or anything about you beyond the email address.</p>
    ...
    <ul>
      <li><b>Cloudflare, Inc.</b> (USA) — hosts the app, runs the database that stores account data and the object store that holds score files you upload, and sits in front of the site as a content-delivery and security network. Cloudflare processes request metadata (IP address, headers, URL, timestamp) to serve and protect the site and keeps standard, short-lived server logs. <a href="https://www.cloudflare.com/privacypolicy/" rel="noopener">Cloudflare's privacy policy</a>.</li>
    ...
    <p>Each item's retention is in the table in §3. In addition: our database keeps a rolling 30-day history for disaster recovery, so deleted data leaves that history within 30 days. Cloudflare's server logs are kept for its own short operational window. When you delete your account we delete your uploaded score files, your practice data, sessions, codes and the account record immediately in one operation.</p>
    ...
    <ul>
      ...
      <li><b>Erasure</b> — <em>delete account</em> removes everything from our servers at once, uploaded score files included; <em>remove from the cloud</em> on a score removes just that file. <em>Sign out &amp; clear this device</em> removes the local copy too.</li>
...
const terms = `
    ...
    <p>Chopinly is a practice assistant for musicians: a practice clock attributed to goals, notes, a history and analytics, plus a metronome, tuner, pitch pipe, sight-singing books and a reader for your own sheet music. It runs in your browser and works without an account. An optional account backs your practice up and syncs it between your devices, and gives you a private space for the score files you choose to upload.</p>
    ...
    <p>You own the practice data, notes, score files and everything else you put into Chopinly. You grant us only the limited license needed to store, back up, sync, transmit and display it to you and your other devices in order to provide the Service. That license ends when you delete the content or your account. We do not sell your content, share it with anyone for their purposes, or use it to train machine-learning models.</p>
    <p><b>Sheet music.</b> Upload only music you have the right to keep a personal copy of — editions you bought, public-domain scores, your own manuscripts and arrangements. Uploaded files are private to your account: they are never shared with other users, listed, indexed or published, and only your own signed-in devices can fetch them. Each account has a storage allowance shown in the app, which we may change with notice. If you are a rights holder and believe a file in someone's private space infringes your rights, write to <a href="mailto:${MAIL}">${MAIL}</a> with enough detail to identify the work and your claim; on a valid notice we remove the file and inform the account holder, who may respond to us in turn.</p>
...
const disclaimer = `
    ...
    <p>The metronome keeps time on your browser's audio clock; the tuner and pitch pipe depend on your device's microphone and speakers and on the room; the sight-singing judge, and the ear trainer when you choose to answer on your own instrument, estimate pitch (and rhythm) from a microphone signal. They are practical aids of good but ordinary accuracy, not calibrated instruments, and their results can be affected by hardware, noise and browser behavior.</p>
